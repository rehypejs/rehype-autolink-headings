 ...
 *   `'before'` (optional).
 * @property {Readonly<Properties> | null | undefined} [properties]
 *   Extra properties to set on the link when injecting (default: `undefined`
 *   or `{ariaHidden: true, tabIndex: -1}` when `'append'` or `'prepend'`).
 * @property {Test | null | undefined} [test]
 *   Test for which headings are linked (optional).
 */
...
/**
 * Add links to headings (h1-h6) back to themselves.
 *
...
export default function rehypeAutolinkHeadings(options) {
  ...
  function inject(node) {
    ...
    node.children[behavior === 'prepend' ? 'unshift' : 'push'](
      create(node, clone(props), children)
    )
  ...
  function around(node, index, parent) {
    ...
    const children = toChildren(content, node)
    const link = create(node, clone(props), children)
  ...
  /** @type {import('unist-util-visit').Visitor<Element>} */
  function wrap(node) {
    node.children = [create(node, clone(props), node.children)]
    return [SKIP]
  }
}
