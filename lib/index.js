 ...
 *
 * @type {import('unified').Plugin<[(Options | undefined)?], Root>}
 */
export default function rehypeAutolinkHeadings(options = {}) {
  ...
  function inject(node) {
    node.children[behavior === 'prepend' ? 'unshift' : 'push'](
      create(node, structuredClone(props), toChildren(content, node))
    )
  ...
  function around(node, index, parent) {
    ...
    const link = create(node, structuredClone(props), toChildren(content, node))
  ...
  /** @type {import('unist-util-visit').Visitor<Element>} */
  function wrap(node) {
    node.children = [create(node, structuredClone(props), node.children)]
    return [SKIP]
  }
